...
#    Updated: 2018/09/14 12:05:05 by mcarney          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
# **************************************************************************** #
...
do
	...
	do
		...
		./corewar ./Champs/$P1 ./Champs/$P2 -dump $1 > ./MCOREWAR_OUTPUT/$OUTPUT
		./r_corewar ./Champs/$P1 ./Champs/$P2 -d $1 > ./RCOREWAR_OUTPUT/$OUTPUT

		DIFF=$(diff ./RCOREWAR_OUTPUT/$OUTPUT ./MCOREWAR_OUTPUT/$OUTPUT)

		if [ "$DIFF" = "" ]; then
			printf "${GREEN}$P1 VS $P2 ✔ ${NC}\n"
		else
			printf "${RED}$P1 VS $P2 ✘ (REAL COREWAR IN FIRST)${NC}\n"
